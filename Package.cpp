...
namespace sdds {
    ...
    void grouptTraces() {  // Fully provided
        ...
        for (y = no_of_traces - 1; y > 0; y--) {
            for (z = 0; z < y; z++) {
                if (users[z].user_id > users[z + 1].user_id) {
                    temp_record = users[z];
                    users[z] = users[z + 1];
                    users[z + 1] = temp_record;
                }
            }
        }
    }
    ...
    void display() {
        ...
        for (int count = 0; count < no_of_traces; count++) {
            if ((users[count].timeinhours > 1.0) && (users[count].dayofweek == 'F')) {
                cout << users[count].user_id << "," << users[count].timeinhours << "," << users[count].Fctime << "," << users[count].Fwifitime << "," << users[count].Package_Name << endl;
            }
        }
        /*cout << "....................................................." << endl;*/
    }
    ...
    void deallocateMemory(void) {
        for (int var = 0; var < no_of_traces; var++) {
            delete[] users[var].Package_Name;
        }
        delete[] users;
    }
